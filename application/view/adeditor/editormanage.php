...
<table  cellpadding="0" cellspacing="0" class="showinfo">
  ...
  <?php
	for( $k=1, $i=$pageoffset+1;$i <= $pageoffset+$rownums ; $i++ ,$k++){
		...
		echo "<td align='center' >
		 	<a href=\"javascript:if(confirm('确实要删除吗?'))location='index.php?controller=adeditor&action=editorinfoshow&f=editorInfoDel&id=$edusername'\"> 删除</a>&nbsp;
		 	<a href='index.php?controller=adeditor&action=editorinfoupdate&f=editorInfo&id=$edusername'>编辑</a>&nbsp;
   ...
   <tr style="text-align: center">
    <td > 
    	...
    	<button type='button' style="margin: 0px 5px 0px 5px">
		 <a href='index.php?controller=adeditor&action=editoradd'> 增加一项</a>
		 </button> &nbsp;&nbsp;
         <button type='button' >
		 <a href='index.php?controller=adeditor&action=editorinfoshow&f=editorInfoShow'> 返回首页 </a>
		 </button>
    	</div>
    </td >
  </tr>
