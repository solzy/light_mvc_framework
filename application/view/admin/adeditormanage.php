...
<form action="index.php?controller=admin&action=adeditormanage&f=delCheckedAdeditor" method="post"> 
<table  cellpadding="0" cellspacing="0" class="showinfo">
  ...
  <?php
	for( $k=1, $i=$pageoffset+1;$i <= $pageoffset+$rownums ; $i++ ,$k++){
		...
		$flag = 1;
		while( $element = each($adeditorinfo[$k-1])){	
		...
		echo "<td align='center' >
		 	<a href=\"javascript:if(confirm('确实要删除吗?'))location='index.php?controller=admin&action=adeditormanage&f=adeditorDel&id=$edusername'\"> 删除</a>&nbsp;
		 	<a href='index.php?controller=admin&action=adeditorupdate&f=oneAdeditorInfo&id=$edusername'>编辑</a>&nbsp;
		 	 <a href='index.php?controller=admin&action=adeditordetail&f=oneAdeditorInfo&id=$edusername'> 查看详细</a>
			</td>";
		echo "</tr>";
	}
?>
   ...
   <tr style="text-align: center">
    <td > 
    	...
    	<button type='button' style="margin: 0px 5px 0px 5px">
		 <a href='index.php?controller=admin&action=adeditoradd'> 增加一项</a>
		 </button> &nbsp;&nbsp;
         <button type='button' >
		 <a href='index.php?controller=admin&action=adeditormanage&f=adeditorInfoM'> 返回首页 </a>
		 </button>
    </td >
  </tr>
</table>
